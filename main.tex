...
\begin{abstract}
Hydrology plays a central role in applied and fundamental environmental sciences, but it is well known to suffer from an overwhelming diversity of models, particularly to simulate streamflow. We discuss here in detail how such diversity did arise based on the example of Switzerland. The case study's relevance stems from the fact that Switzerland, despite of being a small country, shows a variety of hydro-climatological regimes, of water resources management challenges and of hydrological research institutes that led to a model diversification that stands exemplary for the diversification that arose also at much larger scales. Our analysis, based on literature review, personal enquiry and a author survey, summarises the main driving forces behind model diversification and discusses drawbacks and advantages of model diversity. Our analysis shows that convenienceXXXX stands out as the main motivation for model selection in the past, but future pathways might emerge from facilitated collaboration through open sourcing and code-sharing platforms. We anticipate that this review not only helps researchers from other fields but in particular also the international hydrology community to understand why we have so many different models.
\end{abstract}%
...
\begin{deluxetable}{l l l c}
	...
	HBV-light & Hydrologiska Byråns Vattenbalansavdelning - light & lumped or semi-distributed &  E  \\
...
The impact of glacier retreat on hydrology is at the heart of many climate impact studies \citep{Horton2006, Schaefli2007b, Junghans2011, Addor2014, Finger2015, Etter2017}. This gave rise to the development of the GERM model \citep{Huss2016, Junghans2011, Farinotti2012, Finger2013}, which particularly focuses on glacier processes, and a new glacier retreat parameterisation scheme widely applied internationally \citep{Huss2010}, called Δh-parametrisation. However, the awareness of the important limitations stemming from the poor representation of glacier dynamics in hydrological models has triggered an increasing effort in incorporating glacier evolution models. A recent example of the technical implementation of the Δh-parameterisation in HBV-light, which defines the glacier thickness change at a certain elevation in response to an overall change in ice mass in given in \citep{Seibert2018}.
...
One point to note is that the adequacy of the models to a different climate is generally not discussed. References to previous studies are sometimes provided, without the latter having addressed this point explicitly. While it is relatively easy to demonstrate a model's ability to reproduce floods or drought conditions, its transferability to other climate conditions is more difficult to prove directly. 
...
%comment: use info from below
%Action: As Nans showed in his paper, researchers tend to use the model that was developed locally, also at an international level. I would say that Switzerland has the particularity of being an alpine country where glaciers and snow-related processes have an important role, and hydropower infrastructures have a large impact on hydrology. In this context, we developed models that are tailored to our needs, and might be less inclined to use a general-purpose hydrological model. Other places in the world have specific contexts that differ from ours, for example arid regions or countries with a relatively flat topography and might be inclined to develop models tailored to their needs. However, there should be a greater synergy between actors in similar environments… So, why a non-Swiss should read it? Hum… Ideas?

%Nans: It is not because open-source models and modular frameworks were not available, they were. I think there is a need for a better account of the political/human/emotional component of the evolution of hydrological models, and this paper is an ideal candidate for this.

%comBS: what is different from other countries is the fact that there are not so many operationnally use models

%combS: from other review: but I think they could strengthen their study by using their knowledge of the Swiss modelling landscape to analyse more critically the prevailing way to select and develop models (in particular by identifying limitations of the status-quo and barriers to the adoption of open-source and modular models),
