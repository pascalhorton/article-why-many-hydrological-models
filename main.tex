...
%add text piece below

One model currently used in Switzerland has been specifically designed to simulate hydropower operations, RS MINERVE. In this particular case, the development of a specific model was considered necessary to simulate streamflow in the Valais region, which shows a particular high density of hydropower infrastructure. On account of this, it was also used coupled with other hydrological models, such as PREVAH, to include these major influences on the system \citep{Andres2016, Stucki2018}.
...
Access to the model source code has been qualified as a relevant driving factor by 55.1\% of the researchers for previous work, unsurprisingly with a clear contribution from model developers (Fig. \ref{fig:survey_histograms}). This number even increases to 81.6\% for the current situation, which reveals a strong desire to be able to edit the model code to adapt it when necessary. However, despite access to the source code, reasearchers don't necessarily modify it. This might also partly be a psychological factor, i.e. people feel more secure to invest time and energy in a model that they know they can eventually adapt if necessary, rather than being stuck in a situation where they cannot modify the model code. Although access to the model code is important to many, it does not mean that everyone sees the necessity of public availability (as open-source) of the model code. Indeed, 61.2\% of the researchers agreed with that statement, which is high, but less than the 81.6\% of reserachers wchich wished for individual access to the code. Some models have been open-source for a long time, such as VIC or FUSE, but either were used very sporadically for an individual catchment -generally by foreign institutions- or have not yet been used in Switzerland.
